...
Verified against the working tree on 2026-09-14, after the `oceanml3d/` namespace move, the NOSC
transplant, the MONAI trunk switch and the move of the toy family to `oceanml3d/legacy/`. Gate at
that point: `pytest -q -m "not slow"`, `ruff check .` clean.

## How to read the paths

Two model families live side by side, and the distinction is load-bearing:

* `oceanml3d/legacy/models/*.py` — the toy / Lorenz / QG family from `4dvarnet-fm-opencode`, built
  through `oceanml3d/legacy/models/factory.py` (`model_type` dispatch) and trained by
  `legacy/train.py`. It is **in reserve**: still tested, still runnable, out of the way
...
* `oceanml3d/models/ocean/` — the gridded-ocean family from the NOSC port, built through the
  ...
  trained by the `oceanml3d` CLI. This is what the repository is about.
...
|---|---|---|---|
| Lorenz-63 dynamics | `models/lorenz63_dynamics.py`, `data/lorenz63.py` | **done** | both carried over as `oceanml3d/legacy/models/lorenz63_dynamics.py` and `oceanml3d/legacy/data/lorenz63.py`; the port's RK4 registry copy is `oceanml3d/dynamics/lorenz.py` (4th-order convergence tested) |
| Lorenz-96, two-scale (the one the fm case studies use) | `models/lorenz96_dynamics.py` | **done** | `oceanml3d/legacy/models/lorenz96_dynamics.py`, plus `lorenz96_2scale` in `oceanml3d/dynamics/lorenz.py`; golden file pending (PLAN 3.0) |
| Lorenz-96, single-scale (textbook) | `data/lorenz96.py` | **done** | `oceanml3d/legacy/data/lorenz96.py`; `lorenz96` + `config/data/lorenz96.yaml` + `scripts/make_toy_data.py` on the registry side |
...
| Capability | Original | Status | Where / why |
|---|---|---|---|
| Direct U-Net | `models/direct_unet.py`, `unet.py` | **done** | both kept (`oceanml3d/legacy/models/direct_unet.py`, `unet.py`); the registry equivalent is `nosc_unet` (`models/ocean/nn/unet_monai.py`, MONAI `DiffusionModelUNet`; `unet_nosc.py` is the pre-MONAI trunk) with heads/attention options |
...
| MONAI U-Net adapter (1-D) | `models/monai_unet_adapter.py` | **done** | `oceanml3d/legacy/models/monai_unet_adapter.py`; MONAI is now a hard dependency (`monai>=1.5,<1.6`, which installs next to torch 2.4), and the `DiffusionUNetResnetBlock` monkeypatch it needs is shared with the gridded trunk as `models/ocean/nn/unet_monai.py::patch_diffusion_resblock` |
| Model construction | *(in `train.py`)* | **done (redesigned)** | extracted to `oceanml3d/legacy/models/factory.py` during the namespace move: it was imported *from the root script* `train.py`, which only resolves with the repo root on `sys.path`. `legacy/train.py` re-exports it |
| `DynamicsBase` + `get_dynamics()` factory | `models/dynamics.py` | **done (both forms)** | the original `if/elif` factory is `oceanml3d/legacy/models/dynamics.py`; the port's registry version is `oceanml3d/dynamics/base.py`. PLAN 3.0 converges them |
...
| Capability | Original | Status | Where / why |
|---|---|---|---|
| EnKF (inflation, Gaspari-Cohn localisation) | `evaluation/baselines.py::EnKF` | **done** | `oceanml3d/legacy/evaluation/baselines.py::EnKF`; registry counterpart `models/ocean/assimilation/model.py::EnsembleKalmanFilter` (a registered model, so it exports products) |
| ETKF | `::ETKF` | **done** | `oceanml3d/legacy/evaluation/baselines.py::ETKF`; `tests/test_baselines_enkf.py` |
...
| Experiment runner, sweeps, tuning | `evaluation/run*.py`, `batch/*sweep*.py`, `tune_*.py` | **done** | `oceanml3d/legacy/evaluation/{run,run_l96,run_l96_sweep,run_l96_sweep2,run_qg_baselines,sweep_qg_baselines,tune_l96_weak4dvar,experiment,estimate_metrics}.py` all kept; Hydra multirun + `config/ablation/` cover new sweeps |
| Neural inference from a checkpoint | `evaluation/neural_inference.py` | **done** | kept; now builds through `oceanml3d/legacy/models/factory.py` like `legacy/train.py` does, so a model registered in one is usable from the other |
...
|---|---|---|
| Typed config schema (`conf/schema.py`) | **done (two of them)** | `oceanml3d/legacy/conf/schema.py` from the fm side and `oceanml3d/config_schema.py` from the port — see §4 |
...
|---|---|---|
| `oceanml3d/legacy/conf/schema.py` (412 lines, ~25 dataclasses) **and** `oceanml3d/config_schema.py` (146 lines, `validate_config`) | two Hydra entry points: `config/legacy/config.yaml` for `legacy/train.py`, `config/main.yaml` for the `oceanml3d` CLI. Disjoint importers — the first is used by 5 test files, the second only by `cli.py` and `test_config_validation.py` | PLAN 3.x: one schema, or an explicit statement that the two CLIs are permanent |
| `oceanml3d/legacy/models/dynamics.py` (`if/elif` factory) **and** `oceanml3d/dynamics/base.py` (registry) | the fm code and the port each brought a dynamics abstraction | PLAN 3.0 — converge on the registry |
| `legacy/models/fourdvarnet.py` **and** `models/ocean/fourdvarnet/` | genuinely different models (L96 solver vs gridded ocean), same name | **not** a duplication to remove; the `models/ocean/` split is the fix |
| ~~`config/experiment/` mixes 67 toy/L96/QG presets with 11 gridded ones~~ | one flat directory, two pipelines | **resolved 2026-09-14**: the 67 toy presets moved to `config/legacy/experiment/` |
