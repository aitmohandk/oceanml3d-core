...
| | |
|---|---|
| `docs/adding_a_model.md` | ~~chemins d'avant le renommage~~ — **corrigé au lot 4** |
...
- [x] **P1-1** Un seul rang simule (`oceanml3d/io.py::is_global_zero`), les autres attendent ; écritures atomiques via `os.replace`. `prepare_data()` n'était pas utilisable : `dm.setup("fit")` a besoin des fichiers avant `trainer.fit`.
...
- [x] **P2-3** Jitter centré sur ±½ stride, borné des deux côtés.
...
- [x] `test_model_smoke.py` paramétré sur `head` × `time_mode` (6 combinaisons) et `attention_levels` ; plus le rejet d'une tête inconnue et d'une base EOF au mauvais nombre de niveaux.
- [x] Test de bout en bout `prepare-obs → train → predict → export → validate_manifest` sur données synthétiques OSSE-3D, marqué `slow`, avec assertions de non-trivialité (variance spatiale) et de métadonnées CF.
- [x] `nightly.yml` : suite complète (y compris `slow`) chaque nuit, plus `workflow_dispatch` avec un `ref`.
...
- [x] `docs/adding_a_model.md` (chemins `models/ocean/`, section sur ce que fait la classe de base), `docs/product_format.md` (v2 + champs d'ensemble), `docs/data_preparation.md` (moitié OSSE-3D + `prepare-obs`) ; 148 → 168 fait au lot 0.
