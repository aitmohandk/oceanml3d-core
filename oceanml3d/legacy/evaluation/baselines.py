...
from oceanml3d.legacy.models.dynamics import DynamicsBase
...
def _as_numpy(x):
    ...
    `Strong4DVar.assimilate` feeds this accumulator with numpy arrays while `EnKF`/`ETKF` feed it
    tensors; the tensor-only version meant the energy-score path of Strong4DVar raised
    AttributeError and had therefore never run.
    """
...
class Weak4DVar:
    def __init__(
        ...
        if dynamics is None:
            raise ValueError(
                f"{type(self).__name__} requires a `dynamics`: every forward step calls "
                "dynamics.step(). The default of None used to fall back to a hard-coded "
                "Lorenz-63 state_dim of 3, which no longer exists."
            )
