#!/usr/bin/env python3
"""FDV1-warm-started SDA guided sampling on the cached L96 test dataset.
...
Combines two already-trained, frozen models with no retraining:
  ...
  - An SDA prior (``models/sda.py::UnconditionalPriorCFM``/``ConditionalPriorCFM``,
    i.e. SDA1 or SDA2) is sampled via ``evaluation/sda_sampler.sda_guided_sample``,
    but instead of starting from pure noise at tau=0, the trajectory is
    warm-started at ``--tau0`` from ``interpolant.mix(noise, FDV1_estimate, tau0)``
    -- a "SDEdit"-style warm start (see ``sda_guided_sample``'s docstring).
    ``guided_obs_cost``/the Tweedie x_hat_1 machinery are unchanged; only the
    trajectory's starting point differs.
...
Same two-step, scheme-agnostic design as ``eval_sda_l96.py``: no metrics are
computed here, just estimates + truth saved to .npz, then scored generically.
"""
import argparse
import json
import logging
...
from oceanml3d.legacy.evaluation.neural_inference import BatchDict, load_model, prepare_dataset
from oceanml3d.legacy.evaluation.sda_sampler import sda_guided_sample

logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s")
logger = logging.getLogger(__name__)


def _run_case(fdv1_model, sda_model, dataloader, device, obs_var_indices,
...
def main():
    parser = argparse.ArgumentParser(description="Run FDV1-warm-started SDA guided sampling on L96 S0/S1")
    parser.add_argument("--fdv1-checkpoint", required=True, help="Path to FDV1 checkpoint .ckpt")
    parser.add_argument("--fdv1-config", required=True, help="Path to FDV1 config.yaml")
    parser.add_argument("--sda-checkpoint", required=True, help="Path to SDA (SDA1 or SDA2) checkpoint .ckpt")
    ...
    parser.add_argument("--guidance-weight", type=float, default=40.0,
    ...
    parser.add_argument("--output", default="sda_fdv1_hybrid_eval_results.json", help="Output JSON")
    ...
    from oceanml3d.legacy.models.sda import ConditionalPriorCFM
    ...
    for case in args.cases:
        est = _run_case(fdv1_model, sda_model, dataloaders[case], device, obs_var_indices,
