...
from oceanml3d.legacy.data.dataloader import ConcatFMDataset, FlowMatchingDataset, collate_fm, make_collate_fm
from oceanml3d.legacy.data.lorenz63 import Lorenz63Config, make_mixed_datasets, make_s0_s1_trainval
from oceanml3d.legacy.data.random_param_dataset import RandomParamLorenz63Dataset
from oceanml3d.legacy.evaluation.metrics import param_rmse, rmse
from oceanml3d.legacy.models.factory import model_factory  # re-exported: scripts and tests import it from here
from oceanml3d.legacy.training.lightning_module import LitModel
from oceanml3d.legacy.training.pipeline import create_trainer, train_stage

BASE = str(pathlib.Path(__file__).resolve().parents[1])   # repo root: experiments/ lives there
...
def _make_eval_batch(w, device, param_names=("sigma", "rho", "beta", "c1"),
    ...
    from oceanml3d.legacy.data.dataloader import FlowMatchingBatch, _l96_biased_param_vector
    ...
    if param_names == ["F", "c1", "hx", "eps", "w1", "w2", "w3", "w4"]:
        from oceanml3d.legacy.data.dataloader import _l96_true_param_vector
...
@hydra.main(config_path="../config/legacy", config_name="lorenz63_default", version_base="1.3")
def main(cfg: DictConfig):
    ...
    if system == "lorenz96":
        from oceanml3d.legacy.data.lorenz96 import Lorenz96Config, make_l96_s0_s1_trainval
        from oceanml3d.legacy.data.lorenz96 import make_datasets as make_l96_datasets
        NO = dc.get("NO", 8)
        J = dc.get("J", 4)
    ...
    if system == "lorenz96":
        ...
        if dc.get("normalize", False):
            from oceanml3d.legacy.data.normalization import load_norm_stats
