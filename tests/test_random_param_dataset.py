...
class TestRandomParamDataset:
    ...
    def test_getitem_keys(self, tiny_cfg):
        ds = RandomParamLorenz63Dataset(tiny_cfg, param_noise=0.2)
        item = ds[0]
        window = {"true_state", "obs", "obs_mask", "forcing_true", "forcing_corrupted"}
        # the generating parameters, exposed under two names: `sigma`/`rho`/`beta` is what the
        # filters read, `true_*` (plus `true_c1`) is what the joint-estimation path reads.
        params = {"sigma", "rho", "beta", "true_sigma", "true_rho", "true_beta", "true_c1"}
        # kept so the observations of a window can be regenerated without the noise draw
        # depending on iteration order -- see `__getitem__` and `data/dataloader.py`.
        provenance = {"obs_seed"}
        assert set(item.keys()) == window | params | provenance, f"Got keys: {set(item.keys())}"
    ...
    def test_deterministic_with_seed(self, tiny_cfg):
        """`obs` is NaN wherever the mask is false, and NaN != NaN: `allclose` without
        `equal_nan=True` reports every unobserved step as a mismatch, whatever the data.
        The two datasets are in fact bit-for-bit identical, so `equal` is what to assert.
        """
        ds1 = RandomParamLorenz63Dataset(tiny_cfg, param_noise=0.2)
        ds2 = RandomParamLorenz63Dataset(tiny_cfg, param_noise=0.2)
        for i in range(len(ds1)):
            assert torch.equal(ds1[i]["obs_mask"], ds2[i]["obs_mask"])
            for key in ["true_state", "obs"]:
                a, b = ds1[i][key], ds2[i][key]
                assert torch.equal(a.isnan(), b.isnan()), \
                    f"NaN pattern differs at window {i}, key {key}"
                assert torch.equal(a[~a.isnan()], b[~b.isnan()]), \
