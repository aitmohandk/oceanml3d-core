...
class TestDataEquivalence:
    ...
    def test_dataset_reproducible(self):
        ...
        `obs` is NaN at every unobserved step, and NaN != NaN, so the bare `assert_close` this
        used to make reported "141 / 150 (94.0%) mismatched elements" on two datasets that are
        bit-for-bit identical -- 94% is just the fraction of steps `obs_interval` leaves unobserved,
        not a reproducibility failure. `equal_nan=True` compares what was meant.
        """
